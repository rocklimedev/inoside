...
export default function AddVendorForm({ onSuccess }) {
  ...
  const { data: vendorTypes = [], isLoading: loadingTypes } =
    useGetVendorTypesQuery();

  const [createVendorType, { isLoading: creatingType }] =
    useCreateVendorTypeMutation();
  ...
  const handleCreateType = async () => {
    ...
    try {
      await createVendorType({ name }).unwrap();
      ...
      toast.error(err?.data?.message || "Failed to add vendor type");
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!form.type_ids.length) {
      return toast.error("Vendor type is required");
    }
      ...
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-6 pb-6">
        ...
        <div>
          ...
          <div className="flex gap-2">
            <Select
              value={form.type_ids[0] || ""}
              onValueChange={(v) => update("type_ids", [v])}
            >
              <SelectTrigger>
