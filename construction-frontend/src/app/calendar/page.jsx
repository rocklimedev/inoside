...
export default function CalendarPage() {
  ...
  const openEditDialog = (evt) => {
    ...
    setForm({
      ...
      start_time: toLocalInput(evt.start_time),
      end_time: toLocalInput(evt.end_time),
      participants: evt.participants || [],
      description: evt.description || "",
      category: evt.category || "client_meeting",
      color: evt.color || "#ef7f1b",
    });
    setShowEventDialog(true);
  };
  ...
  const handleSave = async () => {
    if (!form.title || !form.start_time || !form.end_time) {
      toast.error("Title and times required");
      return;
    }
    setSaving(true);
    try {
  ...
  const handleDrop = async (evt, targetDate) => {
    ...
    try {
      const res = await api.put(`/calendar/events/${evt.id}`, {
        start_time: formatISO(newStart),
        end_time: formatISO(newEnd),
      });
  ...
  const monthGrid = useMemo(() => {
    ...
    for (let i = 1; i <= dim; i++)
      days.push({ date: i, month, year, isOther: false });
  ...
  const getEventsForDay = (date) =>
    filteredEvents.filter((e) => isSameDay(new Date(e.start_time), date));
    ...
    <div className="flex h-full" data-testid="calendar-page">
            ...
            {monthGrid.map((d, i) => {
              ...
              return (
                ...
                >
                  {d.date}
                  {hasEvents && !isToday && (
                    <span className="absolute bottom-0.5 w-1 h-1 rounded-full bg-[#ef7f1b]" />
                  )}
                </button>
              );
            })}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="p-4 border-b border-gray-200 bg-white flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button
              ...
              className="text-xs font-medium text-[#ef7f1b] border border-[#ef7f1b]/30 px-3 py-1.5 rounded-lg hover:bg-orange-50 transition-colors"
            ...
            <div className="flex border rounded-lg overflow-hidden">
              {VIEWS.map((v) => (
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-medium transition-colors ${viewMode === v.id ? "bg-[#ef7f1b] text-white" : "text-gray-500 hover:bg-gray-50"}`}
                >
                  {v.label}
                </button>
              ))}
        ...
        <ScrollArea className="flex-1">
          ...
          {viewMode === "tiles" && (
            <TilesView events={filteredEvents} onEventClick={openEditDialog} />
          )}
          {viewMode === "list" && (
            <ListViewCal
    ...
    <div className="p-4">
      <div className="grid grid-cols-7 border-l border-t border-gray-200">
        {DAYS.map((d) => (
          <div
            key={d}
            className="text-center text-[10px] font-bold uppercase tracking-wider text-gray-400 py-2 border-r border-b border-gray-200 bg-gray-50"
          >
            {d}
          </div>
        ))}
          ...
          return (
            <div
              ...
              onDrop={(e) => {
                e.preventDefault();
                e.currentTarget.classList.remove("bg-orange-50");
                if (dragEvent && !d.isOther) onDrop(dragEvent, d);
              }}
            ...
            >
              <span
                className={`inline-flex items-center justify-center w-6 h-6 text-xs rounded-full mb-1 ${isToday ? "bg-[#ef7f1b] text-white font-bold" : d.isOther ? "text-gray-300" : "text-gray-700"}`}
              >
                {d.date}
              </span>
    ...
    <div className="p-4">
      <div className="grid grid-cols-8 border-l border-t border-gray-200">
        ...
        {dates.map((d, i) => (
          <div
            key={i}
            className={`text-center py-2 border-r border-b border-gray-200 ${isSameDay(d, today) ? "bg-orange-50" : "bg-gray-50"}`}
          >
            <p className="text-[10px] font-bold text-gray-400">
              {DAYS[d.getDay()]}
            </p>
            <p
              className={`text-sm font-bold ${isSameDay(d, today) ? "text-[#ef7f1b]" : "text-black"}`}
            >
              {d.getDate()}
            </p>
          </div>
        ...
        {hours.map((h) => (
          <React.Fragment key={h}>
            <div className="text-[10px] text-gray-400 text-right pr-2 py-2 border-r border-b border-gray-200 h-14">
              {h > 12 ? h - 12 : h}
              {h >= 12 ? "pm" : "am"}
            </div>
            {dates.map((d, di) => {
              const dayEvents = getEvents(d).filter(
                (e) => new Date(e.start_time).getHours() === h,
              );
              return (
                <div
                  key={di}
                  className="border-r border-b border-gray-200 h-14 p-0.5 relative"
                >
                  {dayEvents.map((e, ei) => (
                    <div
                      key={ei}
        ...
        {hours.map((h) => {
          ...
          return (
            <div key={h} className="flex border-b border-gray-100 min-h-[60px]">
              <div className="w-20 text-xs text-gray-400 text-right pr-3 py-2 shrink-0">
    ...
    <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
      {sorted.map((e, i) => (
        <Card
          key={i}
          onClick={() => onEventClick(e)}
          className="p-4 hover:shadow-md transition-all cursor-pointer border-l-[3px]"
          style={{ borderLeftColor: e.color }}
          data-testid={`tile-event-${i}`}
        >
          ...
          <Badge
            className="mt-2 text-[10px] h-[18px] border"
            style={{
          ...
          <div className="space-y-1.5">
            {evts.map((e, i) => (
              <div
                key={i}
                onClick={() => onEventClick(e)}
                className="flex items-center gap-4 p-3 rounded-lg border border-gray-100 hover:bg-gray-50 hover:shadow-sm transition-all cursor-pointer"
              >
                <div
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-gray-400">
                    {[e.project, e.location].filter(Boolean).join(" · ")}
                  </p>
                </div>
