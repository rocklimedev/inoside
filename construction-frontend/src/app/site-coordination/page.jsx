      ...
      <div className="p-4 md:p-6">
        <div className="relative mb-4">
          ...
          <Input
            ...
            className="pl-10"
            data-testid="site-project-search"
          />
        </div>
...
function SiteWorkspace({ project, api, user, onBack }) {
  ...
  const addNote = async () => {
    ...
    try {
      const r = await api.post("/site-coordination/notes", {
    ...
    <div className="flex flex-col h-full" data-testid="site-workspace">
      ...
      <div className="grid grid-cols-3 md:grid-cols-5 gap-3 p-4 md:px-6 bg-gray-50/50 border-b border-gray-100">
        {[
          ...
          {
            label: "Active Tasks",
            value: d.active_tasks?.length || 0,
            icon: CheckCircle,
          },
          {
            label: "Open Issues",
            value: d.open_issues?.length || 0,
            icon: AlertTriangle,
          },
          {
            label: "Pending Deliveries",
            value: d.pending_deliveries?.length || 0,
            icon: Truck,
          },
          {
            label: "Site Stage",
            value: d.site_info?.stage || project.stage,
            icon: Building2,
          },
        ].map((k, i) => {
          const Icon = k.icon;
          ...
          <ScrollArea className="h-full">
            <div className="p-4 max-w-3xl space-y-4">
                ...
                <Card className="p-4" data-testid="today-report">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    ...
                    <div>
                      ...
                      <p className="text-black mt-0.5">
                        {d.today_report.completion_pct}%
                      </p>
                    </div>
                  </div>
                  {d.today_report.issues_faced && (
                    <div className="mt-3 p-2 bg-red-50 rounded-lg">
                      <p className="text-[10px] font-bold text-[#e31d3b] uppercase">
                        Issues
                      </p>
              ...
              {d.active_tasks?.length > 0 && (
                <Card className="p-4">
                  <h3 className="text-sm font-bold text-black mb-3">
                    Active Tasks ({d.active_tasks.length})
                  </h3>
                  <div className="space-y-2">
                    {d.active_tasks.map((t, i) => (
                      <div
                        key={t.id}
              ...
              {!d.all_issues || d.all_issues.length === 0 ? (
                ...
                d.all_issues.map((iss, i) => (
                  <Card
                    ...
                    data-testid={`site-issue-${i}`}
                  >
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${iss.status === "open" ? "bg-red-50 text-[#e31d3b]" : "bg-green-50 text-green-600"}`}
                    >
                      {iss.status === "open" ? (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-gray-400">
                        ...
                        {iss.target_date ? `· Due ${iss.target_date}` : ""}
                      </p>
                    </div>
                    <Badge
                      className={
          ...
          <ScrollArea className="h-full">
            <div className="p-4 max-w-3xl space-y-4">
              {d.pending_deliveries?.length > 0 && (
                <Card className="p-4">
                  <h3 className="text-sm font-bold text-black mb-3">
                    Pending Deliveries ({d.pending_deliveries.length})
                  </h3>
                  <div className="space-y-2">
                    {d.pending_deliveries.map((pd, i) => (
                      <div
                        key={pd.id}
                        className="flex items-center gap-2 p-2 bg-yellow-50/50 rounded-lg"
                        data-testid={`pending-del-${i}`}
                      >
                  ...
                  <div className="space-y-2">
                    {d.recent_dispatches.map((dp, i) => (
                      ...
                      >
                        ...
                        <Badge
                          className={
                            dp.delivery_confirmed
                              ? "bg-green-50 text-green-700 border-0"
                              : "bg-blue-50 text-blue-600 border-0"
                          }
                        >
                          {dp.delivery_confirmed ? "Delivered" : "In Transit"}
                        </Badge>
                      </div>
                    ))}
                  </div>
              ...
              {!d.pending_deliveries?.length &&
                !d.recent_dispatches?.length && (
                  <div className="text-center py-16">
