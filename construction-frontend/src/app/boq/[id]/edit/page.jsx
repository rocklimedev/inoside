...
export default function BoqEditPage() {
  ...
  const [updateBoq, { isLoading: isUpdatingBoq }] = useUpdateBoqMutation();
  ...
  const { data: existingBoq } = useGetBoqByIdQuery(boqId, {
    skip: !boqId,
  });
  console.log(existingBoq);
  // =====================================================
  // LOAD EXISTING BOQ
  // =====================================================
  ...
  useEffect(() => {
    if (existingBoq) {
      const boqData = existingBoq || existingBoq;

      setSelectedProjectId(boqData.project_id || "");

      setBoq({
        id: boqData.id,
        project_id: boqData.project_id || "",
        boq_category_id: boqData.boq_category_id || "",
        title: boqData.title || "",
        revision_no: boqData.revision_no || "Rev-01",
  ...
  const handleSave = async () => {
    ...
    if (!boq.title.trim()) {
      return toast.error("BOQ Title is required");
    }
    ...
    try {
      const boqPayload = {
        ...
        tax_amount: calculateTotals.tax_amount,

        grand_total: calculateTotals.grand_total,
      };
      ...
      if (!boqIdCreated) {
        throw new Error("Failed to retrieve BOQ ID");
      }
      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-3">
            ...
            <Button
              onClick={handleSave}
              disabled={isCreatingBoq || isUpdatingBoq || isSavingStructure}
            >
              <Save className="mr-2 h-4 w-4" />

              {isCreatingBoq || isUpdatingBoq || isSavingStructure
                ? "Saving..."
                : boqId
                  ? "Update BOQ"
                  : "Save BOQ"}
