...
const SECTIONS = [
  ...
  },
  {
    ...
    icon: Image,
    fields: ["reference_links", "client_likes", "client_dislikes"],
  },
];
...
export default function BriefPage() {
  ...
  if (mode === "client-view" && activeBrief)
    return (
      <ClientBriefView
        brief={activeBrief}
        api={api}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                      ...
                      <button
                        ...
                        className="text-gray-300 hover:text-[#e31d3b] transition-colors"
                        data-testid={`delete-brief-${i}`}
                      >
            ...
            return (
              ...
              >
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-8 h-8 rounded-lg flex items-center justify-center ${prog === 100 ? "bg-green-50 text-green-600" : "bg-orange-50 text-[#ef7f1b]"}`}
                    >
                      {prog === 100 ? (
  ...
  const sections = {
    client: (
      <div className="grid grid-cols-2 gap-4">
        {field("client_name", "Client Name", "input", {
          placeholder: "Full name",
        })}
        {field("client_phone", "Phone Number", "input", {
          placeholder: "+91 ...",
        })}
        {field("client_email", "Email", "input", {
          placeholder: "email@example.com",
        })}
        {field("client_location", "Location", "input", {
          placeholder: "City, State",
        })}
        {field("comm_preference", "Communication Preference", "select", {
          options: ["Email", "WhatsApp", "Phone", "Video Call"],
          full: true,
        })}
      </div>
    ),
    project: (
      <div className="grid grid-cols-2 gap-4">
        {field("project_type", "Project Type", "select", {
          options: ["Residential", "Commercial", "Interior", "Renovation"],
        })}
        {field("project_address", "Project Address", "input", {
          placeholder: "Full address",
        })}
        {field("project_area", "Project Area", "input", {
          placeholder: "e.g. 2500 sq.ft",
        })}
        {field("timeline_expectations", "Timeline Expectations", "textarea", {
          placeholder: "Expected timeline for project completion",
          rows: 2,
        })}
      </div>
    ),
    lifestyle: (
      <div className="grid grid-cols-2 gap-4">
        {field("family_members", "Family Members", "input", {
          placeholder: "e.g. 4 members, 2 kids",
        })}
        {field("usage_requirements", "Usage Requirements", "textarea", {
          placeholder: "Home office, guest rooms, etc.",
          rows: 2,
        })}
        {field("special_needs", "Special Needs", "textarea", {
          placeholder: "Elderly-friendly, pet-friendly, etc.",
          rows: 2,
        })}
        {field("future_expansion", "Future Expansion Plans", "textarea", {
          placeholder: "Plans for future additions",
          rows: 2,
        })}
      </div>
    ),
    design: (
      <div className="grid grid-cols-2 gap-4">
        {field("design_style", "Design Style Preference", "select", {
          options: [
            "Modern",
            "Minimal",
            "Traditional",
        ...
        {field("material_preferences", "Material Preferences", "textarea", {
          placeholder: "Italian marble, teak wood, etc.",
          rows: 2,
        })}
        {field("lighting_preference", "Lighting Preference", "textarea", {
          placeholder: "Natural light, warm tones, etc.",
          rows: 2,
        })}
        {field("color_preferences", "Color Preferences", "textarea", {
          placeholder: "Neutral tones, earth colors, etc.",
          rows: 2,
        })}
      </div>
    ),
    budget: (
      <div className="grid grid-cols-2 gap-4">
        {field("budget_range", "Budget Range", "input", {
          placeholder: "e.g. 50-80 Lakhs",
        })}
        {field("budget_flexibility", "Budget Flexibility", "select", {
          options: ["Fixed", "Slightly Flexible", "Flexible", "No Limit"],
        })}
        {field("priority_areas", "Priority Areas for Investment", "textarea", {
          placeholder: "Kitchen, master bedroom, landscaping...",
          rows: 2,
          full: true,
        })}
        {field(
          ...
          "textarea",
          {
            placeholder: "Areas where client wants to save costs",
            rows: 2,
            full: true,
          },
        )}
    ...
    references: (
      <div className="grid grid-cols-2 gap-4">
        {field("reference_links", "Reference Links", "textarea", {
          placeholder: "Pinterest, Houzz, or any URL links",
          rows: 2,
          full: true,
        })}
        {field("client_likes", "Client Likes", "textarea", {
          placeholder: "Design elements the client appreciates",
          rows: 2,
        })}
        {field("client_dislikes", "Client Dislikes", "textarea", {
          placeholder: "Design elements to avoid",
          rows: 2,
        })}
      </div>
    ),
...
function ClientBriefView({ brief, api, user, onBack }) {
  ...
  const handleRequestChanges = async () => {
    try {
      await api.post(`/briefs/${data.id}/request-changes`, {
        note: changeNote,
      });
  ...
  const handleComment = async () => {
    ...
    try {
      const res = await api.post(`/briefs/${data.id}/comments`, {
        content: comment,
      });
