...
const SECTIONS = [
  {
    ...
    title: "Site Details",
    icon: MapPin,
    fields: ["site_address", "location_coordinates", "site_access_conditions"],
  },
  {
    id: "measurements",
    title: "Site Measurements",
    icon: Ruler,
    fields: ["plot_size", "height_restrictions", "setback_rules"],
  },
  {
    id: "context",
  ...
  },
  {
    ...
    icon: EyeIcon,
    fields: ["soil_condition", "structural_limitations", "risk_factors"],
  },
...
export default function SiteRekiPage() {
  ...
  const fetchItems = async () => {
    try {
      const res = await api.get("/site-rekis");
      setItems(res.data);
    } catch (err) {
      console.error(err);
    } finally {
    ...
    <div className="flex flex-col h-full" data-testid="sitereki-page">
        ...
        <div className="p-4 md:p-6">
          {items.length === 0 ? (
            <div className="text-center py-20">
              ...
              <p className="text-sm text-gray-400">No site reki reports yet.</p>
            </div>
          ) : (
                  ...
                  >
                    ...
                    <div className="text-[10px] text-gray-400 space-y-1">
                      {item.site_address && (
                        <p className="flex items-center gap-1">
                          <MapPin className="w-3 h-3" /> {item.site_address}
                        </p>
                      )}
                      <p>
                    ...
                    <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                      <span className="text-[10px] text-gray-400">
                        {(item.site_photos || []).length} photos
                      </span>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
      ...
      <Dialog open={showNewDialog} onOpenChange={setShowNewDialog}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>New Site Reki</DialogTitle>
          </DialogHeader>
          <div className="py-2">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              className="bg-[#ef7f1b] hover:bg-[#d66e15] text-white"
              data-testid="new-sitereki-submit"
            >
              Create
            </Button>
          </DialogFooter>
...
function SiteRekiForm({ item, api, user, onBack, onGenerated }) {
  ...
  const handleSave = async (data) => {
    setSaving(true);
    try {
      await api.put(`/site-rekis/${item.id}`, data || form);
    } catch (err) {
      console.error(err);
    } finally {
      setSaving(false);
  ...
  const filledCount =
    allFields.filter((f) => form[f] && String(form[f]).trim()).length +
    ((form.site_photos || []).length > 0 ? 1 : 0);
  const totalProgress = Math.round(
    (filledCount / (allFields.length + 1)) * 100,
  );
  ...
  const sectionFields = {
    details: (
      <div className="grid grid-cols-2 gap-4">
        {field("site_address", "Site Address", "input", {
          placeholder: "Full address",
        })}
        {field("location_coordinates", "Location Coordinates", "input", {
          placeholder: "Lat, Long",
        })}
        {field("site_access_conditions", "Site Access Conditions", "textarea", {
          placeholder: "Road access, restrictions...",
          full: true,
        })}
      </div>
    ),
    measurements: (
      <div className="grid grid-cols-2 gap-4">
        {field("plot_size", "Plot Size", "input", {
          placeholder: "e.g. 2400 sq.ft",
        })}
        {field("height_restrictions", "Building Height Restrictions", "input", {
          placeholder: "e.g. G+3 floors",
        })}
        {field("setback_rules", "Setback Rules", "textarea", {
          placeholder: "Front, side, rear setbacks",
          full: true,
        })}
      </div>
    ),
    context: (
      <div className="grid grid-cols-2 gap-4">
        {field("neighboring_buildings", "Neighboring Buildings", "textarea", {
          placeholder: "Describe surroundings",
        })}
        {field("road_width", "Road Width", "input", {
          placeholder: "e.g. 30 ft",
        })}
        {field("sun_orientation", "Sun Orientation", "select", {
          options: [
            "North",
            "South",
            "East",
        ...
        {field("noise_levels", "Noise Levels", "select", {
    ...
    utilities: (
      <div className="grid grid-cols-2 gap-4">
        {field("water_connection", "Water Connection", "select", {
          options: [
            "Available",
            "Partially Available",
            "Not Available",
        ...
        {field("electricity", "Electricity", "select", {
          options: ["Available", "Partially Available", "Not Available"],
        })}
        {field("drainage", "Drainage", "select", {
          options: ["Municipal", "Septic Tank", "Not Available"],
        })}
        {field("internet_connectivity", "Internet Connectivity", "select", {
          options: ["Fiber", "Broadband", "Limited", "Not Available"],
        })}
      </div>
    ),
    observations: (
      <div className="grid grid-cols-2 gap-4">
        {field("soil_condition", "Soil Condition", "textarea", {
          placeholder: "Clay, rocky, sandy...",
        })}
        {field("structural_limitations", "Structural Limitations", "textarea", {
          placeholder: "Any known limitations",
        })}
        {field("risk_factors", "Risk Factors", "textarea", {
          placeholder: "Flooding, seismic, etc.",
          full: true,
        })}
      </div>
    ),
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-400">Module: Site Reki</p>
            </div>
          </div>
...
function SiteRekiClientView({ item, api, user, onBack }) {
  const handleApprove = async () => {
    try {
      await api.post(`/site-rekis/${item.id}/approve`);
