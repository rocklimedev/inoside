...
import {
  ...
  Clock, DollarSign, CreditCard, ArrowLeft, Loader2, Edit3, Trash2
} from 'lucide-react';

const BACKEND = process.env.REACT_APP_BACKEND_URL;

const SECTIONS = [
  { id: 'timeline', title: 'Project Timeline', icon: Clock, fields: ['design_duration', 'execution_duration', 'approval_milestones', 'completion_deadline'] },
  { id: 'cost', title: 'Cost Framework', icon: DollarSign, fields: ['design_cost', 'execution_cost', 'consultation_fees', 'misc_budget'] },
  { id: 'payments', title: 'Payment Milestones', icon: CreditCard, fields: ['advance_payment', 'design_completion_payment', 'execution_stage_payments', 'final_payment'] },
];
...
export default function TimeCostPage() {
  ...
  const fetchData = async () => {
    try { const r = await api.get('/time-costs'); setItems(r.data); }
  ...
  const handleDelete = async (id) => {
    try { await api.delete(`/time-costs/${id}`); setItems(p => p.filter(x => x.id !== id));
  ...
  if (mode === 'form' && active) return <TCForm item={active} api={api} onBack={() => { setMode('list'); fetchData(); }} onGenerated={d => { setActive(d); setMode('document'); }} />;
  if (mode === 'document' && active) return <TCDocument item={active} api={api} user={user} onBack={() => { setMode('list'); fetchData(); }} onEdit={() => setMode('form')} />;

  return (
    <div className="flex flex-col h-full" data-testid="timecost-page">
      <div className="p-4 md:p-6 border-b border-gray-200 bg-white">
        <div className="flex items-center justify-between">
          <div><h1 className="text-xl font-black text-black" data-testid="timecost-title">Time & Cost</h1>
            <p className="text-xs text-gray-400 mt-1">{items.length} estimate{items.length !== 1 ? 's' : ''}</p></div>
          {user?.role !== 'Client' && (
            <Button onClick={() => setShowNew(true)} className="bg-[#ef7f1b] hover:bg-[#d66e15] text-white" size="sm" data-testid="new-timecost-btn">
      ...
      <ScrollArea className="flex-1"><div className="p-4 md:p-6">
        {items.length === 0 ? (
          <div className="text-center py-20"><Clock className="w-12 h-12 text-gray-200 mx-auto mb-3" /><p className="text-sm text-gray-400">No estimates yet.</p></div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {items.map((item, i) => {
              const st = STATUS_MAP[item.status] || STATUS_MAP.draft;
              return (
                <Card key={item.id} className="p-4 hover:shadow-lg hover:border-[#ef7f1b]/20 transition-all cursor-pointer" onClick={() => openItem(item)} data-testid={`timecost-card-${i}`}>
      ...
      <Dialog open={showNew} onOpenChange={setShowNew}>
        <DialogContent className="max-w-sm"><DialogHeader><DialogTitle>New Time & Cost Estimate</DialogTitle></DialogHeader>
          <div className="py-2"><Label className="text-xs font-bold uppercase tracking-wider text-gray-500">Project Name *</Label>
            <Input value={newName} onChange={e => setNewName(e.target.value)} className="mt-1" data-testid="new-timecost-name" /></div>
          <DialogFooter><Button variant="outline" onClick={() => setShowNew(false)}>Cancel</Button>
            <Button onClick={handleCreate} className="bg-[#ef7f1b] hover:bg-[#d66e15] text-white" data-testid="new-timecost-submit">Create</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
...
function TCForm({ item, api, onBack, onGenerated }) {
  ...
  const save = async (d) => { setSaving(true); try { await api.put(`/time-costs/${item.id}`, d || form); } catch {} finally { setSaving(false); } };
  const generate = async () => {
    setGenerating(true);
    try { await api.put(`/time-costs/${item.id}`, form); await api.post(`/time-costs/${item.id}/generate`);
      const u = await api.get(`/time-costs/${item.id}`); toast.success('Document generated'); onGenerated(u.data);
      ...
      <div className="p-4 md:px-6 border-b border-gray-200 bg-white">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div><h1 className="text-base font-bold text-black">{form.project_name}</h1><p className="text-[11px] text-gray-400">Module: Time & Cost</p></div>
          </div>
        ...
        {SECTIONS.map(s => {
          ...
          return (
            <Card key={s.id} className="overflow-hidden">
              <button onClick={() => setOpen(p => ({ ...p, [s.id]: !p[s.id] }))} className="w-full flex items-center justify-between p-4 hover:bg-gray-50 transition-colors text-left">
                <div className="flex items-center gap-3">
              ...
              {isOpen && <div className="px-4 pb-4 pt-1 border-t border-gray-100 animate-fadeInUp">
                <div className="grid grid-cols-2 gap-4">
                  {s.fields.map(f => (
                    <div key={f}>
                      <Label className="text-[10px] font-bold uppercase tracking-wider text-gray-400">{FIELD_LABELS[f] || f.replace(/_/g, ' ')}</Label>
                      <Input value={form[f] || ''} onChange={e => update(f, e.target.value)} className="mt-1" placeholder={s.id === 'cost' || s.id === 'payments' ? 'e.g. ₹5,00,000' : 'e.g. 3 months'} />
                    </div>
                  ))}
                </div>
              </div>}
    ...
    <div className="flex flex-col h-full" data-testid="timecost-document">
      <div className="p-4 md:px-6 border-b border-gray-200 bg-white">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={onBack} className="text-gray-400 hover:text-black"><ArrowLeft className="w-5 h-5" /></button>
            <div><h1 className="text-base font-bold text-black">{item.project_name}</h1><p className="text-[11px] text-gray-400">Time & Cost Estimate</p></div>
      ...
      <ScrollArea className="flex-1 bg-gray-50"><div className="max-w-3xl mx-auto p-6 md:p-8">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-8 border-b-2 border-[#ef7f1b]">
            ...
            <h2 className="text-2xl font-bold text-black mt-4">Time & Cost Estimate</h2>
            <p className="text-sm text-gray-500 mt-1">{item.project_name} &middot; {new Date(item.created_at).toLocaleDateString()}</p>
          </div>
          <div className="p-8 space-y-8">
            {SECTIONS.map((s, i) => (
              <div key={i}>
                <h3 className="text-xs font-bold uppercase tracking-wider text-[#ef7f1b] mb-4">{i + 1}. {s.title}</h3>
                <div className="grid grid-cols-2 gap-4">
                  {s.fields.map((f, j) => (
                    <div key={j} className={!item[f] ? 'opacity-40' : ''}>
                      <p className="text-[10px] uppercase tracking-wider text-gray-400 font-bold">{FIELD_LABELS[f] || f.replace(/_/g, ' ')}</p>
                      <p className="text-sm text-black mt-0.5 font-medium">{item[f] || '—'}</p>
                    </div>
                  ))}
                </div>
              </div>
