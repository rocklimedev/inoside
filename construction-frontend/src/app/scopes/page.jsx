...
const SECTIONS = [
  ...
  },
  {
    ...
    icon: Users,
    fields: ["payment_timelines", "material_approvals", "decision_timelines"],
  },
];
    ...
    <div className="flex flex-col h-full" data-testid="scope-page">
        ...
        <div className="p-4 md:p-6">
          {items.length === 0 ? (
            <div className="text-center py-20">
              ...
              <p className="text-sm text-gray-400">No scope documents yet.</p>
            </div>
          ) : (
      ...
      <Dialog open={showNew} onOpenChange={setShowNew}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>New Scope of Work</DialogTitle>
          </DialogHeader>
          <div className="py-2">
...
function ScopeForm({ item, api, onBack, onGenerated }) {
  ...
  const save = async (d) => {
    setSaving(true);
    try {
      await api.put(`/scopes/${item.id}`, d || form);
    } catch {
    } finally {
      setSaving(false);
    }
  ...
  const generate = async () => {
    setGenerating(true);
    try {
      await api.put(`/scopes/${item.id}`, form);
      await api.post(`/scopes/${item.id}/generate`);
      const u = await api.get(`/scopes/${item.id}`);
  ...
  const fieldMap = {
    services: (
      <Textarea
        ...
        rows={4}
        placeholder="Architecture, Interior, Execution, Consultation..."
      />
    ),
    work_coverage: (
      <Textarea
        value={form.work_coverage || ""}
        onChange={(e) => update("work_coverage", e.target.value)}
        rows={4}
        placeholder="Design services, Site supervision, Vendor coordination, Material selection..."
      />
    ),
    deliverables: (
      <Textarea
        ...
        rows={4}
        placeholder="Concept design, Working drawings, Execution drawings, Site visits..."
      />
    ),
    exclusions: (
      <Textarea
        ...
        rows={4}
        placeholder="Items not included in the scope..."
      />
    ),
    payment_timelines: (
      <Textarea
        value={form.payment_timelines || ""}
        onChange={(e) => update("payment_timelines", e.target.value)}
        rows={2}
        placeholder="Payment schedule and milestones..."
      />
    ),
    material_approvals: (
      <Textarea
        value={form.material_approvals || ""}
        onChange={(e) => update("material_approvals", e.target.value)}
        rows={2}
        placeholder="Material approval process..."
      />
    ),
    decision_timelines: (
      <Textarea
        value={form.decision_timelines || ""}
        onChange={(e) => update("decision_timelines", e.target.value)}
        rows={2}
        placeholder="Decision turnaround expectations..."
      />
    ),
  };
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-400">Module: Scope of Work</p>
            </div>
          </div>
          ...
          {SECTIONS.map((s) => {
            ...
            return (
              <Card key={s.id} className="overflow-hidden">
                ...
                {isOpen && (
                  <div className="px-4 pb-4 pt-1 border-t border-gray-100 animate-fadeInUp space-y-3">
                    {s.fields.map((f) => (
                      <div key={f}>
                        <Label className="text-[10px] font-bold uppercase tracking-wider text-gray-400">
                          {f.replace(/_/g, " ")}
                        </Label>
                        <div className="mt-1">{fieldMap[f]}</div>
                      </div>
                    ))}
                  </div>
                )}
...
function ScopeDocument({ item, api, user, onBack, onEdit }) {
  ...
  const secs = [
    ...
    { title: "Exclusions", value: item.exclusions },
    {
      title: "Client Responsibilities",
      value: [
        item.payment_timelines,
        item.material_approvals,
        item.decision_timelines,
      ]
        .filter(Boolean)
        .join("\n\n"),
    },
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-400">Scope of Work</p>
            </div>
