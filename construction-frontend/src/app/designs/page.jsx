...
export default function DesignPage() {
  ...
  const fetchDesigns = async () => {
    try {
      const r = await api.get(`/designs?project_id=${selectedProject.id}`);
      setDesigns(r.data);
    } catch (e) {
      console.error(e);
    }
  ...
  const filtered = designs.filter((d) => {
    ...
    if (filterStatus !== "all" && d.approval_status !== filterStatus)
      return false;
      ...
      <div className="flex flex-col h-full" data-testid="design-page">
        <div className="p-4 md:p-6 border-b border-gray-200 bg-white">
          ...
          <p className="text-xs text-gray-400 mt-1">
            Select a project to manage designs
          </p>
        </div>
        <div className="p-4 md:p-6">
          <div className="relative mb-4">
            ...
            <Input
              ...
              className="pl-10"
              data-testid="design-project-search"
            />
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-400">
                Design Library &middot; {designs.length} drawings
              </p>
            </div>
          </div>
                  ...
                  >
                    ...
                    <div className="p-3">
                      <div className="flex items-start justify-between mb-1">
                        ...
                        <Badge
                          className={`${st.color} text-[9px] border-0 shrink-0 ml-1`}
                        >
                          {st.label}
                        </Badge>
                      </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-gray-400">
                        {d.category} &middot; {d.version} &middot; by{" "}
                        {d.uploaded_by}
                      </p>
                    </div>
    ...
    <div className="space-y-3 py-2">
      ...
      <div>
        ...
        <div
          ...
          className="mt-1 border-2 border-dashed rounded-lg p-4 text-center cursor-pointer hover:border-[#ef7f1b]/50 transition-colors"
        >
          <input
            ref={fileRef}
            type="file"
...
function DesignDetail({ design, api, user, onBack }) {
  ...
  const handleComment = async () => {
    ...
    try {
      const r = await api.post(`/designs/${data.id}/comments`, {
        content: comment,
      });
  ...
  const handleApprove = async (status) => {
    try {
      const r = await api.post(`/designs/${data.id}/approve`, {
        status,
        remarks,
      });
      setData(r.data);
      setRemarks("");
      toast.success(
        status === "approved" ? "Design approved" : "Changes requested",
      );
    } catch {
      toast.error("Failed");
    }
  ...
  const handleRevise = async (file) => {
    try {
      ...
      const r = await api.post(`/designs/${data.id}/revise`, {
        file_url: upRes.data.url,
        file_id: upRes.data.file_id,
        filename: file.name,
      });
      setData(r.data);
      toast.success("Revised version uploaded");
    } catch {
      toast.error("Failed");
    }
  };
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-400">
                {data.category} &middot; {data.version}
              </p>
            </div>
            ...
            {user?.role !== "Client" && !data.locked && (
              <>
                <input
                  ...
                  className="hidden"
                  onChange={(e) =>
                    e.target.files?.[0] && handleRevise(e.target.files[0])
                  }
                />
                <Button
                  variant="outline"
        ...
        <div className="w-80 border-l border-gray-200 bg-white flex flex-col">
            ...
            <div className="p-4 border-b border-gray-200">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  className="flex-1 text-[#e31d3b] border-red-200 text-xs"
                  data-testid="design-changes-btn"
                >
                  <X className="w-3 h-3 mr-1" /> Changes
                </Button>
              </div>
            </div>
          )}
          ...
          <div className="p-4 border-b border-gray-200 text-sm space-y-2">
            <div>
              ...
              <p className="text-black text-xs">{data.uploaded_by}</p>
            </div>
            <div>
              <p className="text-[10px] uppercase tracking-wider text-gray-400 font-bold">
                Date
            ...
            {data.room_area_tag && (
              <div>
                ...
                <p className="text-black text-xs">{data.room_area_tag}</p>
              </div>
            )}
            {data.description && (
              <div>
                ...
                <p className="text-black text-xs">{data.description}</p>
              </div>
            )}
          </div>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3 max-h-64 overflow-y-auto">
            {(data.approval_log || []).length === 0 ? (
              <p className="text-xs text-gray-400">No approvals yet</p>
            ) : (
              (data.approval_log || []).map((a, i) => (
                ...
                >
                  {a.action === "approved" ? (
                    ...
                    <AlertCircle className="w-4 h-4 text-[#e31d3b] mt-0.5" />
                  )}
                  <div>
                    <p className="text-xs font-medium capitalize">{a.action}</p>
                    <p className="text-[10px] text-gray-400">
                      by {a.by} ({a.role}) &middot;{" "}
