...
const actionIcon = (a) => {
  ...
  if (al.includes("approved"))
    return <CheckCircle className="w-3.5 h-3.5 text-green-600" />;
  if (al.includes("rejected") || al.includes("changes"))
    return <XCircle className="w-3.5 h-3.5 text-[#e31d3b]" />;
  if (al.includes("revised") || al.includes("sent"))
    return <RefreshCw className="w-3.5 h-3.5 text-[#ef7f1b]" />;
...
const actionColor = (a) => {
  ...
  if (al.includes("revised") || al.includes("sent"))
    return "bg-orange-50 text-[#ef7f1b]";
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            <h1
              ...
              data-testid="revision-logs-title"
            >
              Revision Logs
            </h1>
            <p className="text-xs text-gray-400 mt-1">
              {logs.length} revision{logs.length !== 1 ? "s" : ""} tracked
            </p>
          </div>
        </div>
        ...
        <div className="p-4 md:p-6">
          {filtered.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-sm text-gray-400">No revision logs found</p>
            </div>
          ) : (
                      ...
                      >
                        <td className="py-2.5 px-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-medium text-black truncate max-w-[180px]">
                              {log.document}
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="py-2.5 px-3">
                          <Badge
                            className={`${actionColor(log.action)} text-[9px] border-0`}
                          >
                            {log.action}
                          </Badge>
                        </td>
                        ...
                        <td className="py-2.5 px-3 text-xs text-gray-400 truncate max-w-[150px]">
                          {log.note || "—"}
                        </td>
                      </tr>
                    ))}
            ...
            <div className="flex flex-col h-full" data-testid="revision-detail">
              <div className="p-4 border-b border-gray-200">
                ...
                <p className="text-[10px] text-gray-400 mt-0.5">
                  {activeLog.project} &middot; {activeLog.module}
                </p>
              </div>
              <div className="p-4 space-y-4">
                <div className="flex items-center gap-3">
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center ${actionColor(activeLog.action)}`}
                  >
                    {actionIcon(activeLog.action)}
                  </div>
                  <div>
                    ...
                    <p className="text-[10px] text-gray-400">
                      by {activeLog.by} &middot;{" "}
                      {activeLog.date
                        ? new Date(activeLog.date).toLocaleString()
                        : "—"}
                    </p>
                  </div>
                </div>
