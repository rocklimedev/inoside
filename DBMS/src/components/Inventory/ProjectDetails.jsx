...
const ProjectDetails = () => {
  ...
  if (isLoading) return <Spin tip="Loading project..." size="large" />;
  if (isError || !project)
    return (
      <Alert
        message="Error"
  ...
  const { inventories = [], name } = project;
  ...
  const columns = [
    ...
    { title: "In", dataIndex: "in", key: "in", render: (v) => v || "-" },
    { title: "Out", dataIndex: "out", key: "out", render: (v) => v || "-" },
    {
      title: "Receiver",
    ...
    { title: "Vendor", dataIndex: "vendorId", key: "vendorId", ellipsis: true },
    {
      title: "Quantity",
      dataIndex: "quantity",
      key: "quantity",
          ...
          <Card>
            <Space direction="vertical" size="middle" style={{ width: "100%" }}>
              ...
              <Row gutter={[16, 16]}>
                ...
                {["date_added"].map((field) => (
                  <React.Fragment key={field}>
                    <Col xs={12} sm={8} md={6}>
                      <Text strong>{field} From</Text>
                      <DatePicker
                        style={{ width: "100%", marginTop: 4 }}
                        value={
                          dateFilters[`${field}_from`]
                            ? dayjs(dateFilters[`${field}_from`])
                            : null
                        }
                        onChange={(_, dateString) =>
                          setDateFilters((prev) => ({
                            ...prev,
                            [`${field}_from`]: dateString || null,
                          }))
                        }
                        format="YYYY-MM-DD"
                      />
                    ...
                    <Col xs={12} sm={8} md={6}>
                      <Text strong>{field} To</Text>
                      <DatePicker
                        style={{ width: "100%", marginTop: 4 }}
                        value={
                          dateFilters[`${field}_to`]
                            ? dayjs(dateFilters[`${field}_to`])
                            : null
                        }
                        onChange={(_, dateString) =>
                          setDateFilters((prev) => ({
                            ...prev,
                            [`${field}_to`]: dateString || null,
                          }))
                        }
                        format="YYYY-MM-DD"
                      />
        ...
        >
          <Row gutter={[16, 16]}>
            <Col xs={24}>
              ...
              >
                <Input placeholder="e.g. Cement Bag, Steel Rod..." />
              </Form.Item>
            </Col>
            ...
            <Col xs={24}>
              <Form.Item name="in" label="In">
                <Input placeholder="In details" />
              </Form.Item>
            </Col>

            <Col xs={24}>
              <Form.Item name="out" label="Out">
                <Input placeholder="Out details" />
              </Form.Item>
            </Col>
            ...
            <Col xs={24}>
              <Form.Item name="vendorId" label="Vendor">
                <Input placeholder="Vendor ID or name" />
              </Form.Item>
            </Col>
